...
class PracticeQuestion(Base):
    ...
    solution_steps: Mapped[dict] = mapped_column(JSON().with_variant(JSON, "postgresql"), nullable=True)
    difficulty: Mapped[float] = mapped_column(Float, nullable=False, default=0.5)
    hints: Mapped[dict] = mapped_column(JSON().with_variant(JSON, "postgresql"), nullable=True)
    ...
    practice_set = relationship("PracticeSet")
